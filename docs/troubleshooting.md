...
| Report shell creation fails | Missing `ReportSemanticModelId`, invalid semantic model ID, or insufficient Power BI report permissions | Create the semantic model first, confirm the ID, then rerun with `-CreateReportShell -ReportSemanticModelId`. |
